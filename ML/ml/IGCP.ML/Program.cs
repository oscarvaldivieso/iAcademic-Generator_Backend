// Program.cs - Fuente: STG.vw_ofertas_enriched + fallback por modo (OUTER APPLY)
using System;
...
namespace AcademicOfferPredictor
{
    public class Program
    {
        ...
        private const string SOURCE_VIEW = "STG.vw_ofertas_enriched"; // <- NUEVO
...
SELECT
    ...
    COALESCE(NULLIF(LTRIM(RTRIM(bloque_horario)), ''), 'NA') AS bloque_horario,
    ...
    0 AS pre_solicitudes,                          -- no viene, valor seguro
...
FROM {_sourceView};";
        ...
        // ===================== CARGA PRED (ENRICHED + OUTER APPLY) =====================
...
SELECT
  l.mod_codigo, l.cam_codigo, l.doc_codigo, l.nombre_materia,
...
ORDER BY l.mod_codigo, l.cam_codigo,
        ...
        private async Task SaveResultsToDatabaseAsync(string runTag, List<MLFeatures> features, List<EnrollmentPrediction> predictions)
        {
            ...
            try
            {
                ...
                { del.Parameters.AddWithValue("@p", periodo); await del.ExecuteNonQueryAsync(); }
